...
const dynamicMessageTemplates = [
  ...
  "💖 {name} vient de rejoindre nos généreux donateurs !",
  "🥂 {name} trinque à notre bonheur !",
  "✨ {name} ajoute un peu plus de magie à ce jour !",
];
...
function FunMessageBanner({
  ...
  useEffect(() => {
    const getRandomMessage = () => {
      let message = "";
      const isDynamic = dynamicUntil > Date.now() && latestContribution;

      if (isDynamic && latestContribution) {
        const template = dynamicMessageTemplates[Math.floor(Math.random() * dynamicMessageTemplates.length)];
        message = template.replace("{name}", latestContribution.donorName);
      } else {
        const allMessages = [
          ...funMessages.generic,
  ...
  }, [latestContribution, dynamicUntil]);
...
export default function LiveContributions() {
  ...
  const [dynamicUntil, setDynamicUntil] = useState<number>(0);
  ...
  useEffect(() => {
    if (liveData?.latest && liveData.latest.id !== lastContributionId.current) {
      if (lastContributionId.current !== null) {
        ...
        setShowPopup(true);
        // Show dynamic (contributor name) messages for 2 cycles × 7s = 14s
        setDynamicUntil(Date.now() + 14000);
